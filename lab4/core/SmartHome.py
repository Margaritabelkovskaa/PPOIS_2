...
class SmartHome:
    ...
    def __init__(self, name: str):
        print(f"\n{'='*50}")
        print(f"ИНИЦИАЛИЗАЦИЯ SmartHome: {name}")
        print(f"{'='*50}")
        ...
        # ПРИНУДИТЕЛЬНОЕ СОЗДАНИЕ КОМНАТ ПРЯМО ЗДЕСЬ
        print("\n1. СОЗДАЮ КОМНАТЫ...")
        ...
        for room_name in default_rooms:
            ...
            print(f"   ✅ Комната '{room_name}' создана (ID: {room.id[:8]}...)")

        print(f"\n2. ВСЕГО КОМНАТ: {len(self.rooms)}")
    ...
    def load_state(self, filepath: str = "smarthome_state.json") -> None:
        ...
        # НЕ ОЧИЩАЕМ КОМНАТЫ, А ДОБАВЛЯЕМ
